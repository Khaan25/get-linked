  ...
  return (
    <article aria-label={`faq about ${title}`} className="space-y-4 border-b border-tertiary py-4 last:border-0 min568:py-8">
      <div
        ...
        className={cn('text-title flex cursor-pointer items-center justify-between gap-4 text-xl font-medium max360:text-lg', isOpen && 'text-primary')}
      >
        {title}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.p
            ...
            className="overflow-hidden text-base font-normal leading-7 max360:text-sm/6"
          >
            {description}
          </motion.p>
        )}
